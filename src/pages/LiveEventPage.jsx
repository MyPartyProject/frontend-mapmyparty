...
const LiveEventPage = ({ embedded = false }) => {
  ...
  if (loading) {
    return (
      <div className={`${embedded ? "flex-1" : "min-h-screen"} flex items-center justify-center ${embedded ? "" : "bg-gradient-to-br from-background via-surface to-surface"}`}>
    ...
    return (
      <div className={`${embedded ? "flex-1" : "min-h-screen"} flex items-center justify-center ${embedded ? "" : "bg-gradient-to-br from-background via-surface to-surface"}`}>
        <div className="flex flex-col items-center gap-4 text-center max-w-md">
          ...
          <p className="text-foreground text-lg">Failed to load event</p>
          <p className="text-muted-foreground">{error}</p>
          <button
            onClick={() => navigate(-1)}
            className="px-4 py-2 rounded-xl bg-muted border border-border hover:bg-muted transition text-foreground"
          >
            Go Back
          </button>
        </div>
  ...
  const offPlatformSection = (
    <div className="bg-muted border border-border rounded-2xl p-5 shadow-lg shadow-black/5 space-y-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <h3 className="text-lg font-semibold flex items-center gap-2">
            <Ticket className="w-5 h-5 text-warning" /> Off-Platform Tickets
          </h3>
      ...
      {offPlatformError ? (
        <div className="rounded-xl border border-red-400/20 bg-red-500/10 px-4 py-3 text-sm text-destructive">
          {offPlatformError}
        </div>
      ) : null}
      ...
      {offPlatformRecords.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-muted border border-border rounded-xl p-4">
            ...
            <p className="text-2xl font-bold mt-1 text-foreground">{offPlatformTotals.count}</p>
          </div>
          <div className="bg-muted border border-border rounded-xl p-4">
      ...
      <div className="overflow-hidden rounded-2xl border border-border bg-card/80">
        {offPlatformLoading ? (
          ...
          <div className="px-4 py-12 text-center">
            <p className="text-muted-foreground text-sm">No off-platform tickets recorded yet.</p>
            <p className="text-muted-foreground text-xs mt-1">Use the button above to add the first record.</p>
          </div>
        ) : (
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody className="divide-y divide-border">
                {offPlatformRecords.map((record) => (
                  <tr key={record.id} className="hover:bg-muted transition">
                    <td className="py-3 px-4 font-semibold text-foreground">{record.recipientName}</td>
                    ...
                    <td className="py-3 px-4 text-muted-foreground">{number(record.members)}</td>
                    <td className="py-3 px-4 text-success">{currency(record.price)}</td>
                    <td className="py-3 px-4">
                      {record.checkinTime ? (
                    ...
                    <td className="py-3 px-4 text-muted-foreground max-w-[220px] truncate">{record.notes || "-"}</td>
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="p-2 rounded-lg hover:bg-red-500/10 text-muted-foreground hover:text-destructive transition"
                          title="Delete"
                        >
  ...
  const offPlatformModal = offPlatformModalOpen ? (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="w-full max-w-[22rem] sm:max-w-[24rem] max-h-[min(88vh,42rem)] overflow-hidden rounded-xl border border-border bg-card shadow-2xl shadow-black/5 flex flex-col">
        <div className="flex items-start justify-between gap-3 px-4 py-3 border-b border-border shrink-0">
          <div className="min-w-0">
            ...
            <p className="text-[11px] text-muted-foreground mt-1">Fill the required details to save this record.</p>
          </div>
          <button
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-4 py-3">
          <div className="space-y-3">
            <div className="min-w-0">
            <label className="block text-[11px] uppercase tracking-wide text-muted-foreground mb-1.5">Recipient Name *</label>
            <input
              type="text"
              value={offPlatformForm.recipientName}
              onChange={(e) => setOffPlatformForm((current) => ({ ...current, recipientName: e.target.value }))}
              placeholder="e.g. John Doe"
              className="w-full min-w-0 rounded-lg border border-border bg-muted px-3 py-2 text-xs leading-5 text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-amber-400/50"
            />
            </div>

            <div className="min-w-0">
            <label className="block text-[11px] uppercase tracking-wide text-muted-foreground mb-1.5">Ticket Label *</label>
            <input
              type="text"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="min-w-0">
              <label className="block text-[11px] uppercase tracking-wide text-muted-foreground mb-1.5">Members *</label>
              <input
                type="number"
                min={1}
                value={offPlatformForm.members}
                onChange={(e) => setOffPlatformForm((current) => ({ ...current, members: e.target.value }))}
                className="w-full min-w-0 rounded-lg border border-border bg-muted px-3 py-2 text-xs leading-5 text-foreground focus:outline-none focus:ring-2 focus:ring-amber-400/50"
              />
              </div>
              <div className="min-w-0">
              <label className="block text-[11px] uppercase tracking-wide text-muted-foreground mb-1.5">Price *</label>
              <input
                type="number"
                min={0}
                step="0.01"
            ...
            <div className="min-w-0">
            <label className="block text-[11px] uppercase tracking-wide text-muted-foreground mb-1.5">Check-in Time</label>
            <input
              type="datetime-local"
            ...
            <div className="min-w-0">
            <label className="block text-[11px] uppercase tracking-wide text-muted-foreground mb-1.5">Notes</label>
            <textarea
              rows={2}
              ...
              className="w-full min-w-0 rounded-lg border border-border bg-muted px-3 py-2 text-xs leading-5 text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-amber-400/50 resize-none"
            />
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-end gap-2 px-4 py-3 border-t border-border shrink-0">
          <button
            onClick={closeOffPlatformModal}
            className="px-3 py-1.5 rounded-lg border border-border bg-muted text-muted-foreground hover:bg-muted hover:text-foreground transition text-xs"
          >
            Cancel
          </button>
          <button
      ...
      <>
        <div className="flex-1 flex flex-col overflow-hidden text-foreground">
          <main className="flex-1 overflow-y-auto">
            ...
            {/* Hero */}
            <div className="relative overflow-hidden rounded-2xl border border-border bg-gradient-to-r from-red-600/25 via-purple-500/15 to-blue-600/25 shadow-lg shadow-black/5">
              ...
              <div className="relative p-5 lg:p-6 flex flex-col gap-3">
                <div className="flex items-center justify-between gap-3 flex-wrap">
                  <button
                    onClick={() => navigate(-1)}
                    className="flex items-center gap-2 px-4 py-2 rounded-xl bg-muted border border-border hover:bg-muted transition text-sm"
                  >
                ...
                <div className="flex items-center gap-3 flex-wrap text-xs text-muted-foreground uppercase tracking-[0.22em]">
                  <Sparkles className="w-4 h-4 text-destructive" />
                  Live Event Detail
                  <span className="px-2 py-1 rounded-full bg-muted border border-border text-[11px]">
                    {event.eventStatus}
                  </span>
                  <span className="px-2 py-1 rounded-full bg-muted border border-border text-[11px]">
                    {event.publishStatus}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="bg-muted border border-border rounded-2xl p-5 shadow-lg shadow-black/5">
                <p className="text-xs uppercase tracking-wide text-muted-foreground flex items-center gap-2">
                  <Ticket className="w-4 h-4 text-warning" /> Tickets
                </p>
              ...
              <div className="bg-muted border border-border rounded-2xl p-5 shadow-lg shadow-black/5">
                <p className="text-xs uppercase tracking-wide text-muted-foreground flex items-center gap-2">
                  <Users className="w-4 h-4 text-info" /> Booked Users
                </p>
                <p className="text-3xl font-bold mt-2 text-cyan-100">{ticketTotals.sold}</p>
                <p className="text-sm text-muted-foreground">{bookingTotals.totalBookings} bookings</p>
              </div>
              <div className="bg-muted border border-border rounded-2xl p-5 shadow-lg shadow-black/5">
                <p className="text-xs uppercase tracking-wide text-muted-foreground flex items-center gap-2">
                  <Activity className="w-4 h-4 text-success" /> Checked-in
                </p>
                <p className="text-3xl font-bold mt-2 text-emerald-100">{checkInData.total}</p>
                <p className="text-sm text-muted-foreground">{checkInRate}% of booked</p>
              </div>
              <div className="bg-muted border border-border rounded-2xl p-5 shadow-lg shadow-black/5">
                <p className="text-xs uppercase tracking-wide text-muted-foreground flex items-center gap-2">
                  <Clock className="w-4 h-4 text-info" /> Bookings Status
                </p>
                <p className="text-lg font-semibold mt-2 text-foreground">{bookingTotals.confirmed} confirmed</p>
                <p className="text-sm text-muted-foreground">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div className="flex items-center gap-3 bg-muted border border-border rounded-2xl p-4 shadow-lg shadow-black/5">
                <div className="w-10 h-10 rounded-xl bg-emerald-500/20 border border-emerald-400/30 flex items-center justify-center">
                  <Activity className="w-5 h-5 text-success" />
                </div>
                <div className="flex-1">
                  <p className="text-xs uppercase tracking-wide text-muted-foreground">Check-in rate</p>
                  <p className="text-lg font-semibold text-foreground">{checkInRate}%</p>
                </div>
              </div>
              <div className="flex items-center gap-3 bg-muted border border-border rounded-2xl p-4 shadow-lg shadow-black/5">
                ...
                <div className="flex-1">
                  <p className="text-xs uppercase tracking-wide text-muted-foreground">Avg ticket</p>
                  <p className="text-lg font-semibold text-foreground">₹{number(avgTicketPrice)}</p>
                </div>
              </div>
              <div className="flex items-center gap-3 bg-muted border border-border rounded-2xl p-4 shadow-lg shadow-black/5">
                ...
                <div className="flex-1">
                  <p className="text-xs uppercase tracking-wide text-muted-foreground">Open capacity</p>
                  <p className="text-lg font-semibold text-foreground">{openCapacity} seats</p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              <div className="lg:col-span-2 bg-muted border border-border rounded-2xl p-5 shadow-lg shadow-black/5">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-lg font-semibold flex items-center gap-2">
                    <Ticket className="w-5 h-5 text-warning" /> Ticket types
                  </h3>
                      ...
                      {ticketTypes.map((t) => {
                        ...
                        return (
                          <tr key={t.id} className="hover:bg-muted transition">
                            ...
                            <td className="py-3 pr-4 text-muted-foreground">₹{number(t.price)}</td>
                            <td className="py-3 pr-4 text-muted-foreground">{t.totalQty}</td>
                            <td className="py-3 pr-4">
                              <div className="flex items-center gap-2">
                                <span className="text-muted-foreground">{t.soldQty}</span>
                                ...
                                <span className="text-xs text-muted-foreground">{bookedPct}%</span>
                              </div>
                            </td>
                            <td className="py-3 pr-4">
                              <span className={`${t.availableQty === 0 ? "text-destructive" : "text-success"}`}>
                                {t.availableQty}
                              </span>
                            </td>
                          </tr>
              ...
              <div className="bg-muted border border-border rounded-2xl p-5 space-y-4 shadow-lg shadow-black/5">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-semibold flex items-center gap-2">
                    <Activity className="w-5 h-5 text-success" /> Check-in funnel
                  </h3>
                  <span className="text-xs text-muted-foreground flex items-center gap-2">
                    {socketConnected ? (
                      <>
                ...
                <div className="space-y-3">
                  ...
                  <div className="text-xs text-muted-foreground">
                    Last 15 min check-ins: <span className="text-foreground">{checkInData.last15m}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Venue & schedule */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              <div className="bg-muted border border-border rounded-2xl p-5 shadow-lg shadow-black/5 space-y-3">
                <h3 className="text-lg font-semibold flex items-center gap-2">
                  <MapPin className="w-5 h-5 text-info" /> Venue
                </h3>
                <p className="text-sm text-muted-foreground">{venue.name || "Venue"}</p>
              ...
              <div className="bg-muted border border-border rounded-2xl p-5 shadow-lg shadow-black/5 space-y-3">
                <h3 className="text-lg font-semibold flex items-center gap-2">
                  <Calendar className="w-5 h-5 text-warning" /> Schedule
                </h3>
                <p className="text-sm text-muted-foreground">Start: {formatDateTime(event.startDate)}</p>
                <p className="text-sm text-muted-foreground">End: {formatDateTime(event.endDate)}</p>
                <div className="text-xs text-muted-foreground">Day label: {formatDate(event.startDate)}</div>
              </div>
                ...
                {realtimeAddOns && realtimeAddOns.length > 0 ? (
                  <div className="space-y-2 max-h-32 overflow-y-auto">
                        ...
                        <div className="flex items-center gap-3 ml-2">
                          <div className="flex items-center gap-2 text-xs">
                            <span className="text-muted-foreground">
                              <span className="text-success font-medium">{item.receivedQty}</span>
                              <span className="text-muted-foreground"> / </span>
                              <span className="text-muted-foreground">{item.totalQty}</span>
                            </span>
                            <span className={`font-medium ${
                              item.remainingQty <= 0 ? "text-destructive" :
                              item.consumptionRate >= 80 ? "text-warning" : "text-muted-foreground"
                            }`}>
                    ...
                    {realtimeAddOns.length > 4 && (
                      <p className="text-xs text-muted-foreground">+{realtimeAddOns.length - 4} more items</p>
                    )}
                  </div>
                ) : (
            ...
            <div className="bg-muted border border-border rounded-2xl p-5 shadow-lg shadow-black/5">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-lg font-semibold flex items-center gap-2">
                  <Users className="w-5 h-5 text-info" /> Bookings health
                </h3>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="bg-muted border border-border rounded-xl p-4">
                  ...
                  <p className="text-2xl font-bold mt-1 text-destructive">{bookingTotals.cancelled}</p>
                </div>
                <div className="bg-muted border border-border rounded-xl p-4">
                  <p className="text-xs uppercase tracking-wide text-muted-foreground">Total bookings</p>
                  <p className="text-2xl font-bold mt-1 text-foreground">{bookingTotals.totalBookings}</p>
                </div>
              </div>
            </div>
            {offPlatformSection}
  ...
  return (
    <>
      <div className="min-h-screen lg:h-screen flex flex-col lg:flex-row bg-gradient-to-br from-background via-surface to-surface text-foreground">
      {/* Sidebar */}
      <aside
        className={`${sidebarOpen ? "w-64" : "w-24"} flex-shrink-0 h-full lg:h-screen lg:sticky lg:top-0 bg-card border-r border-border flex flex-col transition-all duration-300`}
      >
        <div className="p-4 border-b border-border flex items-center justify-between">
          <h1 className={`text-2xl font-extrabold tracking-tight ${sidebarOpen ? "block" : "hidden"}`}>
            ...
            <span className="text-foreground">MyParty</span>
          </h1>
          <button
        ...
        <div className="mt-auto p-4 border-t border-border">
          <div className="relative bg-gradient-to-br from-white/5 via-white/0 to-blue-500/5 border border-border rounded-xl p-3 shadow-lg shadow-black/5">
            <div className="flex items-center gap-3 w-full text-left hover:bg-muted transition rounded-lg px-2 py-1">
              ...
              {sidebarOpen && (
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">Live control</p>
                </div>
              )}
