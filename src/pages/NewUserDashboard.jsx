...
const NewUserDashboard = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background text-foreground">
      {/* ── Desktop Header ── */}
      <header className="hidden lg:flex items-center justify-between h-16 px-6 bg-card/80 backdrop-blur-lg border-b border-border fixed top-0 left-0 right-0 z-40">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <Input
              ...
              className="w-full pl-9 pr-3 h-9 bg-muted border-border text-foreground text-sm placeholder:text-muted-foreground rounded-lg focus:ring-1 focus:ring-ring/50 focus:border-primary/50"
            />
            {searchOpen && normalizedSearchQuery.length >= DASHBOARD_SEARCH_MIN_LENGTH && (
              <div className="absolute right-0 top-full z-50 mt-2 w-[24rem] overflow-hidden rounded-xl border border-border bg-card text-foreground shadow-2xl">
                <div className="border-b border-border px-3 py-2">
                  <p className="truncate text-xs text-muted-foreground">
                    Search results for <span className="text-muted-foreground">"{normalizedSearchQuery}"</span>
                  </p>
                </div>
                ...
                <button
                  ...
                  className="flex w-full items-center justify-between border-t border-border px-3 py-2 text-left text-sm text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
                >
          ...
          <Button variant="ghost" size="icon" className="relative h-9 w-9 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted">
            ...
            <span className="absolute top-1.5 right-1.5 h-1.5 w-1.5 rounded-full bg-primaryCTA"></span>
          </Button>

          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className="h-9 pl-1.5 pr-3 rounded-lg bg-muted hover:bg-muted border border-border">
                <Avatar className="h-6 w-6">
                  <AvatarImage src={userInfo.avatar} />
                  <AvatarFallback className="bg-primaryCTA text-inverse text-[10px] font-bold">
                    {getInitials(userInfo.name, userInfo.email)}
                  </AvatarFallback>
                </Avatar>
                <span className="hidden xl:block text-sm font-medium text-muted-foreground ml-2 max-w-[120px] truncate">
                  {userInfo.name}
                </span>
                <ChevronDown className="h-3.5 w-3.5 text-muted-foreground ml-1" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent className="w-56 bg-card border-border text-foreground rounded-xl shadow-2xl" align="end">
              <DropdownMenuLabel className="font-normal px-3 py-3">
                <div className="flex items-center gap-3">
                  <Avatar className="h-9 w-9">
                    <AvatarImage src={userInfo.avatar} />
                    <AvatarFallback className="bg-primaryCTA text-inverse text-xs font-bold">
                      {getInitials(userInfo.name, userInfo.email)}
                    </AvatarFallback>
                  </Avatar>
              ...
              <DropdownMenuItem onClick={() => navigate("/dashboard/profile")} className="cursor-pointer text-muted-foreground hover:text-foreground focus:text-foreground focus:bg-muted px-3 py-2">
                <UserIcon className="mr-2.5 h-4 w-4" />
                Profile
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => navigate("/dashboard/bookings")} className="cursor-pointer text-muted-foreground hover:text-foreground focus:text-foreground focus:bg-muted px-3 py-2">
                <Ticket className="mr-2.5 h-4 w-4" />
                My Bookings
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => navigate("/dashboard/support")} className="cursor-pointer text-muted-foreground hover:text-foreground focus:text-foreground focus:bg-muted px-3 py-2">
              ...
              <DropdownMenuItem
                ...
                className="cursor-pointer text-destructive hover:text-destructive focus:text-destructive focus:bg-red-500/10 px-3 py-2 disabled:opacity-50"
              >
      ...
      {/* ── Mobile Header ── */}
      <header className="lg:hidden flex items-center justify-between h-14 px-4 bg-card/90 backdrop-blur-lg border-b border-border fixed top-0 left-0 right-0 z-40">
        ...
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="icon" className="relative h-8 w-8 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted">
            <Bell className="h-4 w-4" />
            <span className="absolute top-1 right-1 h-1.5 w-1.5 rounded-full bg-primaryCTA"></span>
          </Button>
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
            ...
            <DropdownMenuContent className="w-52 bg-card border-border text-foreground rounded-xl shadow-2xl" align="end">
              ...
              <DropdownMenuItem
                onClick={handleLogout}
                disabled={isLoggingOut}
                className="cursor-pointer text-destructive focus:text-destructive focus:bg-red-500/10 px-3 py-2 disabled:opacity-50"
              >
        ...
        <div className="flex items-center justify-around h-16 px-2">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <Icon className={`h-5 w-5 ${active ? 'text-accent-foreground' : ''}`} />
                <span className={`text-[10px] font-medium ${active ? 'text-accent-foreground' : ''}`}>{item.name}</span>
              </Link>
            );
          })}
        </div>
      ...
      <Dialog open={locationPopoverOpen} onOpenChange={setLocationPopoverOpen}>
        <DialogContent className="max-w-[900px] w-[92vw] md:w-[90vw] bg-background border border-border text-foreground rounded-2xl p-6 space-y-5 max-h-[75vh] overflow-y-auto">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-base font-semibold text-foreground">Pick your location</p>
              <p className="text-sm text-muted-foreground">Choose a city or state to filter events near you.</p>
            </div>
            {locationLoading && <Loader2 className="h-5 w-5 animate-spin text-accent-foreground" />}
          </div>
          ...
          <Input
            ...
            className="bg-muted border-border text-foreground placeholder:text-muted-foreground focus:ring-1 focus:ring-ring/50"
          />

          <div className="space-y-3">
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Popular cities</p>
                ...
                return (
                  <button
                    ...
                    className={`flex flex-col items-center gap-1 rounded-xl px-2 py-3 transition-all text-sm border ${
                      active
                        ? "border-primary bg-primaryCTA/10 text-foreground"
          ...
          <div className="space-y-3">
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">All states</p>
